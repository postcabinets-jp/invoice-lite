                  ...
                  <tr key={inv.id} className="hover:bg-zinc-50/60 transition-colors">
                    <td className="px-5 py-3">
                      <Link href={`/invoices/${inv.id}`} className="font-mono text-xs text-zinc-700 hover:text-zinc-900">
                        {inv.number}
                        {inv.is_estimate && (
                          <span className="ml-1.5 text-[10px] text-zinc-400 font-sans">[見積]</span>
                        )}
                      </Link>
                    </td>
                    ...
                    <td className="px-5 py-3 text-zinc-500 tabular-nums">{formatDate(inv.issue_date)}</td>
                    <td className="px-5 py-3 text-zinc-500 tabular-nums">{formatDate(inv.due_date)}</td>
                    <td className="px-5 py-3 text-right font-medium tabular-nums text-zinc-900">
                      {formatCurrency(inv.total ?? 0, inv.currency)}
                    </td>
                    <td className="px-5 py-3">
                      <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_CLASS[inv.status as InvoiceStatus] ?? 'bg-zinc-100 text-zinc-500'}`}>
                        {STATUS_LABEL[inv.status as InvoiceStatus] ?? inv.status}
                      </span>
                    </td>
                    <td className="px-5 py-3">
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        {inv.status === 'draft' && (
                          <Link href={`/invoices/${inv.id}/send`}>
                            <Button variant="ghost" size="icon" className="h-7 w-7">
